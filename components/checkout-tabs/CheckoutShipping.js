...
const { countryTax, enableCountry, customRates } = projectSettings;
...
class CheckoutShipping extends React.Component {
      ...
      getShippingRates(data)
        .then((res) => {
          if (res.data.status) {
            ...
            const uspsRates = isUspsRates
              ? rates.filter((el) => el.carrier === "USPS")
              : rates;
            const shippingOptions = filterShippingRates(uspsRates).sort(
              (a, b) => a.rate - b.rate
            );
  ...
  changeShippingMethod = (method) => {
    const { isCustomRates } = this.state;
    const { customRate, id, service } = method;
    const { setShippingCharge, setShippingType } = this.props;
    const shippingType = service;
    const shpId = isCustomRates
      ? "custom_rates"
      : this.state.shippingAllResponse.id;
  ...
  render() {
    ...
    const { email, address, shippingRates } = this.state;
        ...
        <Form onSubmit={this.onSubmit}>
                ...
                <Form.Item>
                  {shippingRates.length > 0 &&
                    getFieldDecorator("shippingMethod", {
                      ...
                      >
                        {shippingRates.map((el, i) => (
                          <Radio key={i} value={el}>
                            ${el.customRate.toFixed(2)} ({el.customName})
                          </Radio>
                        ))}
                      </Radio.Group>
                    )}
          ...
          <TitleList versions={["sm-border"]} parentClass={componentClass}>
            <div className="row" style={{ justifyContent: "center" }}>
              <Button parentClass="c-checkout" type="submit" theme="dark">
                Continue
              </Button>
            </div>
          </TitleList>
...
export default connect(
  mapStateToProps,
  mapActionToProps
)(Form.create({ name: "checkoutShipping" })(CheckoutShipping));
