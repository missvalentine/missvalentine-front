...
class OrderListItem extends React.Component {
  render() {
    const { versions, parentClass, order, serial, reorder } = this.props;
    ...
    const { products, grandTotal, createdOn, _id } = order;
    ...
    return (
      <div className={className}>
        <div serial={serial} className={`${componentClass}__inner`}>
          <div
            data-title="products"
            className={`${componentClass}__title-wrapper`}
          >
            {products.map((el, key) => {
              return (
                <Link key={key}>
                  <a
                    className={`${componentClass}__link ${componentClass}__link--title`}
                    href={`/shop/${el.title.replace(/ /g, "-")}`}
                  >
                    {el.title}
                  </a>
                </Link>
          ...
          <div className={`${componentClass}__actions`}>
            ...
            <div className={`${componentClass}__action`}>
              <a
                href={`${projectSettings.invoiceUrl}/${_id}.pdf`}
                target="_blank"
                className={`${componentClass}__link`}
              >
                Invoice
        ...
        <div className={`${componentClass}__review-wrapper`}>
          {isReviewRemain && (
            <Link href="#">
              <a
                className={`${componentClass}__link ${componentClass}__link--underline-gold`}
              >
